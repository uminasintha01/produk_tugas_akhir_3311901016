    ...
    this.storage.get(tokenKey).then((token) => {
      this.sellingApiService.getBrandLaptop(token).subscribe(
        (response: any) => {
         ...
         this.data_ukuran_laptop = response.data.ukuran_laptop;
         this.data_os = response.data.os;
         this.data_jenis_storage = response.data.jenis_storage;
